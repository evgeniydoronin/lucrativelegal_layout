...
function initializeAstronautDrawing() {
    ...
    // Ищем элементы космонавта
    const astronautContainer = document.querySelector('.astronaut-container');
    const drawElements = document.querySelectorAll('.astronaut-svg .draw-me');
    
    console.log('🔍 Контейнер космонавта найден:', !!astronautContainer);
    console.log(`🔍 Найдено .draw-me элементов: ${drawElements.length}`);

    if (drawElements.length === 0) {
        console.error('❌ Элементы .draw-me в космонавте не найдены!');
        return;
    }

    // Логируем каждый элемент
    drawElements.forEach((element, index) => {
        console.log(`📝 Элемент ${index + 1}:`, element.tagName, element.getAttribute('stroke'));
    });
    ...
    gsap.set(".astronaut-svg .draw-me", { drawSVG: "0%" });
    ...
    const tl = gsap.timeline({
        scrollTrigger: {
            ...
            onUpdate: (self) => {
                console.log(`🚀 Сегментированная прорисовка: ${Math.round(self.progress * 100)}%`);
            }
        },
